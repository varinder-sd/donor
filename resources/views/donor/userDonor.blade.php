                           ...
                           <table id="dataTable" class="table table-striped table-bordered table-hover" style="width:100%">
								...
								@foreach($all as $row)
									<tr>
										...
										<td>
										   @if(!empty($row->donor_picture)) 
										   <img src="{{ asset($row->donor_picture) }}" width="82" style="display: flex;">
										    @endif
										   <h6 style="text-transform: capitalize;"> {{ $row->name }}</h6>
										    </td>									
										<td>{{ $row->covid_postive_date }}</td>
										<td>{{ $row->alternate_mobile_number }}</td>
										<td><a href="{{route('admin.statususerddonor', ['id' => $row->user->id])}}"> {{ $row->user->name }}</a></td>
										<td>{{ $row->blood->name }}</td>
										<td>
										@if(isset($row->city->name))
                                            {{ $row->city->name }} , {{ $row->state->name }}
                                        @else 
										...
										<label class="switch">
											<input  type="checkbox" class="status" data-id="{{ $row->id }}" name="status" @if($row->donor_status) checked @endif>
